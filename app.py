...
def get_scores(playerID):
    ...
    bearer_token = headers.get('Authorization') # Extract the bearer token from the request headers --> Bearer Token
    #return bearer_token
    token=bearer_token.split() [1] #JUST THE TOKEN (without "Bearer")

    player_types_labels, health_literacy_score, response_pt, response_hl, metrics_overview_pt, metrics_overview_hl = dt.get_digital_twin(playerID, token)
