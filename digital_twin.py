...
def get_digital_twin(player_id, auth_bearer, 
    ...
    if id_latest_record_pt==0 or (id_latest_record_pt!=int((json.loads(response_pt.text))[-1]["id"])):
        parsed_metrics_from_sugarvita = pt_hl.parse_json_sugarvita(response_pt, response_hl, id_latest_record_pt, id_latest_record_hl)
        ...
        if metrics_overview_pt_sugarvita==None and metrics_overview_hl_sugarvita==None:
            (metrics_overview_pt_sugarvita,metrics_overview_hl_sugarvita) = pt_hl.manipulate_initial_metrics_sugarvita(parsed_metrics_cleaned)
        else:
            #print(id_latest_record_pt,"!=",(json.loads(response_pt.text))[-1]["id"])
            (metrics_overview_pt_sugarvita_new_data,metrics_overview_hl_sugarvita_new_data,) = pt_hl.manipulate_initial_metrics_sugarvita(parsed_metrics_cleaned)
    ...
    if id_latest_record_trivia==0 or (id_latest_record_trivia!=int((json.loads(response_trivia.text))[-1]["id"])):
        parsed_metrics_from_trivia = pt_hl.parse_json_trivia(response_trivia, id_latest_record_trivia)
